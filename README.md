...
[무신사(musinsa.com)](https://www.musinsa.com)를 대상으로 한 **Playwright** **단위 테스트** 저장소입니다. Page Object 패턴으로 화면별 로케이터·액션을 분리하고, 로그인 셋업 프로젝트가 생성한 `storageState`(`authed.json`)로 인증이 필요한 시나리오를 반복 실행합니다. 테스트 케이스는 스프레드시트의 시나리오 ID(예: `FEATURE_검색_0xx`)와 코드의 `test` 제목을 맞춰 두었습니다.

### 링크

| 구분 | URL |
|------|-----|
| **Local Test Report** (Vercel에 올린 HTML 리포트) | https://playwright-reports-eight.vercel.app/ |
| **Test Case Sheet** (시나리오·ID 정의) | https://docs.google.com/spreadsheets/d/1MV3zCdSLq4Eb7XZCN4ltTse4kCpJpTJE1423JpVSCIM/edit?gid=0#gid=0 |

로컬에서 `yarn test` 후 생성되는 HTML 리포트는 `playwright-reports/`이며, `yarn playwright show-report playwright-reports`로도 볼 수 있습니다.
...
[`.github/workflows/playwright.yml`](.github/workflows/playwright.yml)에서 `main` / `master`에 대한 **push·pull request**마다 **`ubuntu-latest`**에서 테스트를 실행합니다.
...
**전체 테스트** (`setup` 프로젝트가 먼저 실행되어 로그인 후 `tests/fixtures/storage/authed.json`을 갱신하고, 이어서 `*.spec.ts`가 `chromium` 프로젝트로 실행됩니다.)
